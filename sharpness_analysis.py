...
def memory_size_score(image_size, vouched):
    thresholds = SCORING_THRESHOLDS["memory"]
    if image_size < thresholds["min"]:
        return 0, vouched
    elif image_size <= thresholds["max"]:
        return ((image_size - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
    else:
        vouched += 1
        return 1000, vouched

def score_laplacian(value, vouched):
    thresholds = SCORING_THRESHOLDS["laplacian"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
    else:
        vouched += 1
        return 1000, vouched

def score_sobel(value, vouched):
    thresholds = SCORING_THRESHOLDS["sobel"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
...
def score_local_gradient(value, vouched):
    thresholds = SCORING_THRESHOLDS["local_gradient"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
...
def score_fft(value, vouched):
    thresholds = SCORING_THRESHOLDS["fft"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
...
def score_wavelet(value, vouched):
    thresholds = SCORING_THRESHOLDS["wavelet"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
...
def score_dct(value, vouched):
    thresholds = SCORING_THRESHOLDS["dct"]
    if value < thresholds["min"]:
        return 0, vouched
    elif value <= thresholds["max"]:
        return ((value - thresholds["min"]) / (thresholds["max"] - thresholds["min"])) * 1000, vouched
