...
def process_image(image_path, images):
    try:
        ...
        brisque_score = brisq.get_score(image_uint8)
        ...
        laplacian_score = score_laplacian(laplacian_value, vouched)
        sobel_score = score_sobel(sobel_value, vouched)
        local_gradient_score = score_local_gradient(local_gradient_value, vouched)
        fft_score = score_fft(fft_value, vouched)
        wavelet_score = score_wavelet(wavelet_value, vouched)
        dct_score = score_dct(dct_value, vouched)

        total_score = (((laplacian_score + sobel_score + local_gradient_score + fft_score + wavelet_score + dct_score) / 6) + ((brisque_score*10)/2))
        ...
        return {
            ...
            "BRISQUE": round(float(brisque_score), 2),
            "Vouched": vouched,
...
def move_image_to_category(image_path, result, output_folder):
    ...
    vouched_subfolder = os.path.join(vouched_folder, str(result["Vouched"])) if result[
                                                                                    "Vouched"] > 0 else vouched_folder
